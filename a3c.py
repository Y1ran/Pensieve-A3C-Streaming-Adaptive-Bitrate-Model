...
#            split_5 = tflearn.fully_connected(inputs[:, 5:6, -1], 521, activation='relu')
#            split_6 = tflearn.fully_connected(inputs[:, 6:7, -1], 521, activation='relu')
#            split_7 = tflearn.fully_connected(inputs[:, 7:8, -1], 521,activation='relu')
#            split_8 = tflearn.fully_connected(inputs[:, 8:9, -1], 521,activation='relu')
#            split_9 = tflearn.fully_connected(inputs[:, 9:10, -1], 521,activation='relu')
#            split_10 = tflearn.fully_connected(inputs[:, 10:11, -1], 521,activation='relu')
#            split_11 = tflearn.fully_connected(inputs[:, 11:12, -1], 521,activation='relu')
#            split_12 = tflearn.fully_connected(inputs[:, 12:13, -1], 521,activation='relu')
##            split_13 = tflearn.fully_connected(inputs[:, 13:14, -1], 521,activation='relu')


            act_merge_net = tflearn.merge([split_0, split_1, split_2,split_3, split_4],#,split_5, split_6,\
                                     #split_7,split_8,split_9,split_10,split_11,split_12], \
                                       'concat')
            
            dense_net = tflearn.fully_connected(act_merge_net, 128, activation='relu')
            out = tflearn.fully_connected(dense_net, self.a_dim, activation='softmax',name="NN1_output")
            return inputs, out
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        with tf.variable_scope('critic'):
            inputs = tflearn.input_data(shape=[None, self.s_dim[0], self.s_dim[1]],name="critic_inputs")#, self.s_dim[2]])
...
##            split_13 = tflearn.fully_connected(inputs[:, 13:14, -1], 521,activation='relu')
            ...
            dense_net = tflearn.fully_connected(act_merge_net, 128, activation='relu')
            out = tflearn.fully_connected(dense_net, 1, activation='softmax',name="NN_output")

            return inputs, out
        
    def train_by_ML(train_data, train_label):
        import lightgbm as lgb
        model = lgb.LGBMRegressor(num_leaves=31)
        X_train = np.reshape(np.array(train_data),(-1,1))
        y_train = np.reshape(np.array(train_label),(-1,1))
        model.fit(X_train, y_train)
#        y_pred = model.predict(X_val)
             
        return model

    def moving_avrage(windows):
        avg = 0.99 * windows[-1] + 0.95 * windows[-2] + 0.9 * windows[-3] + \
                     0.8 * windows[-4] + 0.75 * windows[-5] + 0.65 * windows[-6] + \
                     0.55 * windows[-7] + 0.35 * windows[-8] + 0.25 * windows[-9] + \
                     0.15 * windows[-10] + 0.1 * windows[-11] + 0.05 * windows[-12] \
                     + 0.025 * windows[-13] + 0.01 * windows[-14] + 0.001 * windows[-15]
        avg = avg / 15
